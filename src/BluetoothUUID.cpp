...
BluetoothUUID::BluetoothUUID(const char str[]) {
    ...
    /* 16bit or 32bit UUID: number + base UUID */
       uuid[0] = (uint64_t)strtoul(str, NULL, 16) << 32 | 0x00001000ULL;
    ...
    /* 128bit UUID */
       ...
       if (u[8] == '-') {
            u[8] = ' ';
            uuid[0] = (uint64_t)strtoul(u + 0, NULL, 16) << 32;
       } else {
            throw std::invalid_argument(err_msg);
       }
       if (u[13] == '-') {
            u[13] = ' ';
            uuid[0] = uuid[0] | (uint64_t)strtoul(u + 9, NULL, 16) << 16;
       } else throw std::invalid_argument(err_msg);
       if (u[18] == '-') {
            u[18] = ' ';
            uuid[0] = uuid[0] | (uint64_t)strtoul(u + 14, NULL, 16);
       } else throw std::invalid_argument(err_msg);

       if (u[23] == '-') {
            u[23] = ' ';
            uuid[1] = (uint64_t)strtoul(u + 19, NULL, 16) << 48;
       } else throw std::invalid_argument(err_msg);
